...
const InDepthReport = ({ start, end, dataset })=> {
  ...
  function getReport() {
    workingSet(true);
    Meteor.call('buildKPInDepthReport', start, end, dataset, (err, reply)=> {
      err && console.log(err);
      if(reply) {
  ...
  const title = dataset === 'live' ? 'Live' : 'Completed';

...
const PhaseBuilder = ({ app, lockout })=> {
  
  const [ phasesState, phasesSet ] = useState( new Set(app.phases) );
  ...
  function addStep(e) {
    e.preventDefault();
    const newPhaseVal = this.newPhase.value;
    
    let list = phasesState; 
    
    list.add( newPhaseVal ); // add step to list
    
    phasesSet( list ); // update state with the new list
  }
  */

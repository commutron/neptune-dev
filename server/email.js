...
function sendExternalEmail(to, cc, subject, date, body, product, sales, foot, flvr, fine, plainbody) {
...
Meteor.methods({
  ...
  sendTestEmail(to, subject) {
    ...
    if(check(to) && check(from) && check(subject)) {
      ...
      sendExternalEmail( to, cc, subject, date, body, product, sales, foot, flvr, fine, plainbody );
    }
  },
  ...
  handleDevMonitorEmail() {
    ...
    if(emailGlobal && to) {
      ...
      const appstate = JSON.stringify({
        ...
        helpDocs: app.helpDocs,
        timeClock: app.timeClock
      });
      const appmin = JSON.stringify({
        branches: app.branches.length,
        qtTasks: app.qtTasks.length,
        trackOption: app.trackOption.length,
        lastTrack: app.lastTrack.key,
        countOption: app.countOption.length,
        nonConOption: app.nonConOption.length,
        nonConTypeLists: app.nonConTypeLists.length,
        missingType: app.missingType,
        repeatOption: app.repeatOption.length,
        alterFulfillReasons: app.alterFulfillReasons.length,
        tagOption: app.tagOption.length,
        latestSerial: app.latestSerial,
        priorityScale: app.priorityScale,
        lockType: app.lockType,
        nonWorkDays: app.nonWorkDays.length
      });
      ...
      sendInternalEmail(to, subject, date, title, body, "xxxxx", foot, link, fine);
  ...
  handleExtOrderEmail(accessKey, emailPrime, emailSecond, isW, salesOrder, salesEnd) {
    ...
    if(emailGlobal) {
      ...
      */
      ...
      sendExternalEmail( to, cc, subject, date, body, product, salesOrder, foot, flvr, fine, plainbody );
  ...
  handleExtUpdateEmail(accessKey, emailPrime, emailSecond, isW, salesOrder, salesEnd) {
    ...
    if(emailGlobal) {
      ...
      const plainbody = `Ship Due Date has been changed for your order — ${salesOrder} — of — ${product}.`;
  ...
  handleExtRelEmail(accessKey, emailPrime, emailSecond, isW, salesOrder, salesEnd, caution) {
    ...
    if(emailGlobal) {
      ...
      const plainbody = `Your order — ${salesOrder} — of — ${product} — has Entered Production.`;
